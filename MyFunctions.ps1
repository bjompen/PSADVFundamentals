...
function GetUserData {
    ...
    try {
        ...
        Write-Error "Failed to find database $MyUserListFile. Is the path wrong?"
    }
    catch {
        Throw $_
    }
...
function Add-CourseUser {
    ...
    Param (
        ...
        [Parameter(Mandatory)]
        [ValidatePattern('^[A-Z][\w\-\s]*$', ErrorMessage = 'Names must start with capital letters.')]
        [string]$Name,
...
function Confirm-CourseID {
    ...
    # Regex matches all Ids that contains something that is _not_ a digit.
    $AllUsers.Where({$_.Id -notmatch '^\d+$'})
}
